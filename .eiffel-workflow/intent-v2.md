...
- **R3 (Q5, AMENDED Phase 2 / ISSUE 12):** Never-raises boundary wording becomes "tofu-but-valid": unrecoverable native failure on a non-empty item synthesizes a tofu run (glyph id 0 per char, advance pixel_size/2, **a trivial one-to-one cluster map, reversed for RTL items**) + note — never an empty item, never a dropped range. *Amendment:* the original said "identity clusters", which is impossible — an identity map violates `GLYPH_SHAPER.clusters_monotone_rtl` for any RTL item of 2+ characters. The contract won; the binding text was wrong and would have misled a Phase-4 implementer following R3's letter.
...
- **R7 (Q10, AMENDED Phase 2 / ISSUE 7):** Counter definitions bound: shape_calls = run-producing shapes; fallback_probes = probe shapes; disjoint; both zero on cache hits; **fallback probes counted by the calling engine FROM `FALLBACK_CHOICE.probes_performed`; never inside seam doubles.** *Amendment:* the original "counted in facade/engines, never in seam effectings" was unimplementable — the walk-and-probe loop lives inside LIST_FONT_FALLBACK, and the caller sees only the returned choice, so the count could never be correct. `FALLBACK_CHOICE` now carries `probes_performed` (>= 0, invariant); the facade adds it in via `SHAPING_STATISTICS.record_fallback_probes`; seam doubles return 0 because they probe nothing. R7's rationale — doubles stay dumb — is preserved exactly; only its mechanism changed.
- **R8 (Q11, AMENDED Phase 2 / ISSUE 2 — now BOUND, was aspirational):** LAYOUT_CACHE entries carry their full key tuple; hits verify text, width, pixel size **and the FONT_LIST digest the entry was built under**; a mismatch on any of the four demotes to a miss. *Amendment:* R8 always listed the fonts digest in the entry tuple, but the Phase-1 entry stored only the layout, so verification covered text/width/size and the font policy went unchecked — a colliding key could serve a layout computed under a different policy, invisibly. `LAYOUT_CACHE` now stores the digest beside the layout (`digests`, model query `digests_model`, domain pinned to `cache_model`'s by invariant) and `put` / `item_verified` / `has_verified` all carry it. This is belt AND braces: `FONT_LIST.digest` and `SIMPLE_SHAPING.cache_key` are independently injective (length-prefixed components, ISSUE 2), so the key alone can no longer collide across policies either.
...
- **R11 (Phase 2 / ISSUE 4 — seam 4 carries the per-call policy):** `FONT_FALLBACK.font_for` gains a fourth argument: `font_for (a_text: READABLE_STRING_32; a_item: SCRIPT_ITEM; a_requested: SHAPING_FONT; a_policy: FONT_LIST): FALLBACK_CHOICE`, with `require policy_usable: not a_policy.is_empty`. `NULL_FONT_FALLBACK` ignores it; `LIST_FONT_FALLBACK` walks `a_policy` and **no longer holds a creation-time FONT_LIST at all** (`make (a_probe: GLYPH_SHAPER; a_registry: FONT_REGISTRY)`); `SIMPLE_SHAPING.layout` passes its per-call `a_fonts`. Rationale: without the parameter, `layout (text, w, n, my_fonts)` advertised a layout "under policy `a_fonts`" and keyed the cache by its digest while the fallback walk used the list captured at facade creation — and `set_default_fonts` never rewired it, so AC-4's "first covering FONT_LIST font" named a list the consumer no longer held. Seam signatures freeze after Phase 2, so this was settled now. **Gate: Larry's Phase 3 approval — APPROVED 2026-09-02 ("R11 approved, run /eiffel.tasks on simple_shaping").**
